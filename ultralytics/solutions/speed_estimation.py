...
class SpeedEstimator:
    ...
    def plot_box_and_track(self, track_id, box, cls, track):
        ...
        if track_id in self.dist_data:
            self.detected_cars[track_id] = self.names[int(cls)]
    ...
    def calculate_speed(self, trk_id, track):
        ...
        if self.trk_previous_times[trk_id] != 0 and direction != "unknown" and trk_id not in self.trk_idslist:
            ...
            if time_difference > 0:
                ...
                # direction
                distance = track[-1][1] - self.trk_previous_points[trk_id][1]
                if distance < 0:
                    self.detected_directions[trk_id] = "Out"
                else:
                    self.detected_directions[trk_id] = "In"
                
                
                # get current time
                current_time = datetime.datetime.now()
                self.detected_date[trk_id] = current_time.strftime("%m/%d/%Y %H:%M:%S")
                self.detected_year[trk_id] = current_time.year
                self.detected_month[trk_id] = current_time.month
                self.detected_day[trk_id] = current_time.day
                self.detected_time[trk_id] = current_time.strftime("%H:%M:%S")
    ...
    def estimate_speed(self, im0, tracks, region_color=(255, 0, 0)):
        ...
        for box, trk_id, cls in zip(self.boxes, self.trk_ids, self.clss):
            ...
            self.detected_number[trk_id] = ocr_image(im0, box)
